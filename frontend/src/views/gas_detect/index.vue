<template>
  <section class="page" data-module="gas_detect">
    <header class="page-head">
      <div>
        <h2>气体监测管理</h2>
        <p class="page-desc">维护监测点位，围绕点位编号、监测气体、所在管沟、当前浓度做登记、筛选与状态流转。</p>
      </div>
      <div class="page-actions">
        <button class="btn primary" type="button" @click="openCreate">登记监测点位</button>
...
const actions = ["确认预警", "处置报警", "恢复在线"]
const statuses = ["正常", "预警值", "报警值", "离线"]
const stats = [{"label": "正常点位", "value": 0}, {"label": "预警点位", "value": 0}, {"label": "报警点位", "value": 0}]
...
function openCreate() {
  errorMessage.value = '监测点位登记入口尚未接入审批流'
}
...
async function runAction(action: string, row: Row) {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error('气体监测动作未生效，请稍后重试')
    }
    await reload()
  } catch (error) {
    errorMessage.value = error instanceof Error ? error.message : '气体监测操作失败'
  }
}
