"""病害管理接口：维护病害记录，覆盖确认病害、派发修复、销项闭单等动作。"""
...
router = APIRouter(prefix="/api/defect", tags=["病害管理"])

service = DefectService()

LIST_FIELDS = ["病害编号", "关联管段", "病害类型", "发现方式", "严重程度", "定位坐标", "现场照片", "病害状态"]
STATUSES = ["待确认", "一确", "待修复", "已修复", "已闭单"]


@router.get("", response_model=PageResult[dict])
def list_entries(
    keyword: str | None = Query(default=None, description="按病害编号检索"),
    status: str | None = Query(default=None, description="待确认、一确、待修复、已修复、已闭单"),
    page: int = 1,
    size: int = 20,
) -> PageResult[dict]:
    """按病害编号与状态过滤病害管理列表；没有数据时返回空页，不报错。"""
    if size > 200:
...
def run_action(entry_id: int, payload: EntryPayload) -> ActionResult:
    """对单条病害记录执行确认病害、派发修复、销项闭单；不允许的动作会被拦下并说明原因。"""
